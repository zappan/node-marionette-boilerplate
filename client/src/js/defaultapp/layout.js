define(['jquery', 'underscore', 'backbone', 'backbone.marionette', 'util/logger'
  ...
  initialize = function() {
    var layout = app.DefaultApp.layout = new Layout();

    layout.listenTo(layout, 'show', function() {
      log.debug('[defaultapp.layout] layout displayed');
      app.vent.trigger(layoutEvts.defaultAppRendered);
    });

    log.debug('[defaultapp.layout] layout initialized');
